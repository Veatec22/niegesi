...
# I2 Localization jest w tej grze wkompilowane w Assembly-CSharp.dll.
GAME_REFERENCES = [
    'mscorlib.dll',
    'System.dll',
    'System.Core.dll',
    'netstandard.dll',  # ta gra celuje w .NET Standard 2.1
    'UnityEngine.dll',
...
def package(work: Path) -> Path:
    ...
    with zipfile.ZipFile(out, 'w', compression=zipfile.ZIP_DEFLATED) as archive:
        ...
        archive.write(work / 'NieGesiShotgunCopMan.dll', f'BepInEx/plugins/{PLUGIN_FOLDER}/NieGesiShotgunCopMan.dll')
...
def main() -> int:
    ...
    compile_plugin(args.game.resolve(), work / 'NieGesiShotgunCopMan.dll')
    archive = package(work)

    print(json.dumps({
        'version': VERSION,
        'terms': terms,
        'plugin_bytes': (work / 'NieGesiShotgunCopMan.dll').stat().st_size,
        'package': str(archive),
