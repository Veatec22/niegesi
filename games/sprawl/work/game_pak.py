 ...
 def extract(self,path):
  ...
  with self.path.open('rb') as f:
   ...
   if not hasattr(self,'oodle'):
    dll=ctypes.CDLL(r'C:\Users\wazag\Downloads\1adf4055f07a93543ea445f5e34b0de0b3bed5df\Output\.data\oodle-data-shared.dll')
...
if __name__=='__main__':
 ...
 print(p.mount,len(p.files))
 Path('games/sprawl/work/file-list.txt').write_text('\n'.join(p.files),encoding='utf-8')
 en=p.extract('Sprawl/Content/Localization/Game/en/Game.locres')
 assert en==Path('games/sprawl/translations/en.locres').read_bytes(); print('Oodle extraction matches original English')
 for name in p.files:
  if any(s in name.lower() for s in ['language','localiz','flag','mainmenu','main_menu','settings','gameinstance']) and not '/Audio/' in name:
   print(name)
