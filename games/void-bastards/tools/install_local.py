...
itself (another mod, a different BepInEx); the receipt in backups/void-bastards lists every
...
Folders and logs that BepInEx creates at runtime (config, cache, LogOutput.log) remain.
    ...
    .venv\\Scripts\\python.exe games\\void-bastards\\tools\\install_local.py --game "C:\\SteamLibrary\\steamapps\\common\\Void Bastards"
    .venv\\Scripts\\python.exe games\\void-bastards\\tools\\install_local.py --game "..." --restore
"""
import argparse
import hashlib
import json
...
REPO = ROOT.parents[1]
EXE = 'Void Bastards.exe'
RECEIPT = REPO / 'backups' / 'void-bastards' / 'install-receipt.json'
...
def main():
    ...
    package = args.package or max((ROOT / 'dist').glob('Void-Bastards-PL-*.zip'), key=lambda p: p.stat().st_mtime)
