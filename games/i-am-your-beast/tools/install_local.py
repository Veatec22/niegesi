...
itself (another mod, a different BepInEx); the receipt in backups/i-am-your-beast lists every
...
Folders and logs that BepInEx creates at runtime (config, cache, LogOutput.log) remain.
    ...
    .venv\\Scripts\\python.exe games\\i-am-your-beast\\tools\\install_local.py --game "..." --restore
"""
import argparse
import hashlib
import json
...
REPO = ROOT.parents[1]
EXE = 'I Am Your Beast.exe'
RECEIPT = REPO / 'backups' / 'i-am-your-beast' / 'install-receipt.json'
...
def main():
    ...
    package = args.package or max((ROOT / 'dist').glob('I-Am-Your-Beast-PL-*.zip'), key=lambda p: p.stat().st_mtime)
