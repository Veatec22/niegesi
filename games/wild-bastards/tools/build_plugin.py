"""Zbuduj paczkę z pluginem BepInEx: polski dla Wild Bastards bez podmiany plików gry.
...
VERSION = '0.2.0'
DATA = 'WildBastards_Data'
PLUGIN_FOLDER = 'notgeeseWildBastards'
...
# Unity 2021.3 (Mono, .NET Standard 2.1); plugin kompiluje się wprost na bibliotekach gry.
GAME_REFERENCES = [
    'mscorlib.dll',
    'System.dll',
    'System.Core.dll',
    ...
    'Assembly-CSharp.dll',  # I2 Localization i UIMGMainMenu są wkompilowane tutaj
]
...
def package(work: Path) -> Path:
    out = ROOT / 'dist' / f'Wild-Bastards-PL-{VERSION}.zip'
    ...
    with zipfile.ZipFile(out, 'w', compression=zipfile.ZIP_DEFLATED) as archive:
        ...
        archive.write(work / 'notgeeseWildBastards.dll', f'BepInEx/plugins/{PLUGIN_FOLDER}/notgeeseWildBastards.dll')
...
def main() -> int:
    ...
    compile_plugin(args.game.resolve(), work / 'notgeeseWildBastards.dll')
    archive = package(work)

    print(json.dumps({
        'version': VERSION,
        'terms': terms,
        'plugin_bytes': (work / 'notgeeseWildBastards.dll').stat().st_size,
        'package': str(archive),
