...
itself (another mod, a different BepInEx); the receipt in backups/wild-bastards lists every
...
Folders and logs that BepInEx creates at runtime (config, cache, LogOutput.log) remain.
    ...
    .venv\\Scripts\\python.exe games\\wild-bastards\\tools\\install_local.py --game "C:\\SteamLibrary\\steamapps\\common\\Wild Bastards"
    .venv\\Scripts\\python.exe games\\wild-bastards\\tools\\install_local.py --game "..." --restore
"""
import argparse
import hashlib
import json
...
REPO = ROOT.parents[1]
EXE = 'WildBastards.exe'
RECEIPT = REPO / 'backups' / 'wild-bastards' / 'install-receipt.json'
...
def main():
    ...
    package = args.package or max((ROOT / 'dist').glob('Wild-Bastards-PL-*.zip'), key=lambda p: p.stat().st_mtime)
