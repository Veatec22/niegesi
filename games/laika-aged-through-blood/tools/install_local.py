...
itself (another mod, a different BepInEx); the receipt in backups/laika-aged-through-blood lists every
...
Folders and logs that BepInEx creates at runtime (config, cache, LogOutput.log) remain.
    ...
    .venv\\Scripts\\python.exe games\\laika-aged-through-blood\\tools\\install_local.py --game "..." --restore
"""
import argparse
import hashlib
import json
...
REPO = ROOT.parents[1]
EXE = 'Laika Aged through Blood.exe'
RECEIPT = REPO / 'backups' / 'laika-aged-through-blood' / 'install-receipt.json'
...
def main():
    ...
    package = args.package or max((ROOT / 'dist').glob('Laika-Aged-Through-Blood-PL-*.zip'), key=lambda p: p.stat().st_mtime)
