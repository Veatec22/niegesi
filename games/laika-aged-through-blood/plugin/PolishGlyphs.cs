...
//
// Najpierw próbujemy prawdziwych liter: font dynamiczny dorysowuje je sam, a do statycznego
// dokładamy zapasowy font dynamiczny z pliku TTF tego samego kroju, jeśli gra go ma.
// Dopiero gdy liter nie ma nigdzie, dla każdego fontu, któremu
...
namespace NieGesi.Laika
{
    internal static class PolishGlyphs
    {
        ...
        private static bool Patch(TMP_FontAsset font, out bool final)
        {
            ...
            // Font dynamiczny dorysowuje litery w locie: najpierw prawdziwe polskie z pliku kroju,
            // a jeśli ich tam nie ma — bazowe do składania.
            if (font.atlasPopulationMode == AtlasPopulationMode.Dynamic && font.sourceFontFile != null)
            {
                ...
                {
                    Plugin.Log.LogInfo("Font TMP „" + font.name + "”: polskie litery dorysowuje sam (font dynamiczny).");
                    return false;
                }
                var bases = "IlLE";
            ...
            if (Wanted(own).Length == 0)
            {
                Plugin.Log.LogInfo("Font TMP „" + font.name + "”: ma polskie litery.");
                return false;
            }
        ...
        /// Litery z przepisów, których font nie ma.
        ...
        /// Plik TTF tego samego kroju, jeśli gra już go wczytała. Tylko wczytane: Resources.LoadAll
        /// wczytuje wszystkie zasoby z Resources, łącznie z prefabami scen, i wywraca Laikę (0.1.1).
        ...
        /// Zapasowy font dynamiczny z pliku kroju: prawdziwe litery zamiast składanych.
        private static bool RealFallback(TMP_FontAsset font, Dictionary<uint, TMP_Character> own)
        {
            ...
            if (font.fallbackFontAssetTable == null) font.fallbackFontAssetTable = new List<TMP_FontAsset>();
            font.fallbackFontAssetTable.Insert(0, fallback);
            Plugin.Log.LogInfo(string.Format("Font TMP „{0}”: litery {1} z pliku kroju „{2}”{3}.", font.name, needed, source.name,
                string.IsNullOrEmpty(absent) ? "" : ", bez " + absent));
            return true;
        }
